...
function generateFlightList() {
    console.log(flights);
    console.log("generateFlightList  ");
    for (var i = 0; i < flightslist.length; i++) {
        var flights = "";
        console.log("HELLO");
        // console.log(JSON.stringify(flightslist));
        flights = flights + "FLIGHT location: " + flightslist[i].location + "      Date/time:  " + flightslist[i].time + "\n";
        console.log(flights);
        $('#flights_list').append("FLIGHT location: "+flightslist[i].location+"      Date/time:  "+flightslist[i].time);
